package br.com.trustsystems.persistence.dao.test;


import br.com.trustsystems.persistence.util.ReflectionUtils;
import br.com.trustsystems.persistence.dao.test.tasks.IDataInitializationTask;
import br.com.trustsystems.persistence.dao.test.tasks.RequiredDataInitialization;
...
public abstract class JUnitTest 
{
    ...
    public void setUp() {
        ...
        logger.info("Data initialization setup took "
    ...
    public void tearDown() {
        long start = System.currentTimeMillis();
        try {
            // exceptions in test setup lead to strange behaviour
            // so it's necessary to catch them here
            tearDownTasks();
        } catch (Exception e) {
            e.printStackTrace();
        ...
        logger.info("Data initialization tear down took "
                + (System.currentTimeMillis() - start) + " ms");
    }
    ...
    public void setUpTasks() {
        for (IDataInitializationTask task : activeDataInitTasks) {
            logger.info("Running setup of initialization task "
    ...
    public void tearDownTasks() {
        ...
        for (int i = activeDataInitTasks.size() - 1; i >= 0; i--) {
            ...
            logger.info("Running tear down of initialization task "
                    + task.getClass().getSimpleName());
            task.tearDown();
        }
    }
