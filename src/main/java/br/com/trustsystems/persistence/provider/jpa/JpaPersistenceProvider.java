...
import br.com.trustsystems.persistence.dao.query.Options;
import br.com.trustsystems.persistence.dao.query.LockType;
...
/**
 * An implementation of {@link br.com.trustsystems.persistence.dao.IPersistenceProvider} interface for JPA 2 persistence technology.
 *  
 */
public abstract class JpaPersistenceProvider implements IPersistenceProvider
{
    ...
    private <D extends Persistent<? extends Serializable>> D persistInternal(Class<D> domainClass, D domainObject)
	{
        ...
        if (isPersistent(domainObject)) 
		{
            ...
            entityManager().flush(); //FIXME: Necessary for OrphanRemoval=true
	...
	private void configureQuery(javax.persistence.Query query, Query.ParametrizedQuery source)
	{
		...
		if(source.isFirstResultSet())query.setFirstResult(source.getFirstResult());
	}
